...
public class Block {
	...
	public void Draw() //그리는 방법으로 앞위뒤옆아래위의 값을 그린다. 
	{
		if(bType == BlockType.AIR) return; //블럭타입이 공기면 그냥 안그린다.

		if(!HasSolidNeighbour((int)position.x,(int)position.y,(int)position.z + 1)) //각각의 면을 채워나간다.
